...
/* Skip exactly two lines (--- and +++). Returns 1 on EOF. */
static int
skip_two_lines (FILE *f)
{
	int ch;
...
open_rej_file (const char *file, struct rej_file *rej)
{
	...
	/* Open the .rej file */
	xasprintf (&rej_file, "%s.rej", file);
	rej->fp = xopen (rej_file, "r");
	free (rej_file);

	/* Skip the --- / +++ lines to get to the start of the @@ line */
	if (skip_two_lines (rej->fp))
		error (EXIT_FAILURE, errno,
		       "Failed to find any hunks in .rej file");
		   ...
		   unsigned long **hunk_offs, const char *unline)
{
	...
	if (unline) {
		...
		xasprintf (&out_file, "%s.patch", file);
...
fuzzy_cleanup (const char *file, int rej)
{
	...
	xasprintf (&tmp, "%s.patch", file);
	unlink (tmp);

	/* Remove the .rej file if one was generated */
	if (rej) {
		sprintf (tmp, "%s.rej", file);
		unlink (tmp);
	}
	free (tmp);
}
...
filter_edge_hunks (FILE *in)
{
	...
	for (line = fbuf; (line = memchr (line, '\n', end - line));) {
		...
		for (size_t i = from; i < to; i++) {
			...
			if (from || to < nlines) {
				/* Edges were trimmed; regenerate the @@ line
				 * with adjusted line counts. */
				unsigned long os, oc, ns, nc;

				if (read_atatline (atat.s, &os, &oc, &ns, &nc))
					error (EXIT_FAILURE, 0,
					       "line not understood: %s",
					       atat.s);

				fprintf (out, "@@ -%lu,%lu +%lu,%lu @@\n",
					 os + ntop[0], oc - ntop[0] - nbot[0],
					 ns + ntop[1], nc - ntop[1] - nbot[1]);
			} else {
